...
class ScienceRAG:
    ...
    def retrieve_context(self, query, marks=None, chapter=None, n_results=5):
        """
        Retrieve relevant context from vector database
        
        Args:
            query: User's question
            marks: Optional filter for specific mark questions (e.g., "one", "two")
        ...
        if chapter:
            where_filter["chapter"] = str(chapter)
    ...
    def generate_answer(self, query, marks=None, chapter=None):
        ...
        results = self.retrieve_context(query, marks, chapter)
        ...
        for i, doc in enumerate(results['documents'][0]):
            ...
            # Add subtopic info if available
        ...
        chapter_info = f" from Chapter {chapter}" if chapter else ""
        
        prompt = f"""You are a science teacher helping students prepare for exams.

Context from textbook{chapter_info}:
{context}

Student's Question{marks_info}: {query}

Please provide a clear and accurate answer based on the context provided. If the question asks for a specific number of marks, ensure your answer is appropriately detailed for that mark value.
...
Answer:"""
    ...
    def interactive_mode(self):
        ...
        print("  - Add '[Chapter Y]' to filter by chapter")
        print("  - Example: 'What is ecosystem? [2 marks] [Chapter 13]'")
        ...
        while True:
            try:
                ...
                # Extract chapter
                chapter_match = re.search(r'\[chapter\s*(\d+)\]', user_input, re.IGNORECASE)
                if chapter_match:
                    chapter = chapter_match.group(1)
                    user_input = re.sub(r'\[chapter\s*\d+\]', '', user_input, flags=re.IGNORECASE).strip()
                
                print("\n⏳ Generating answer...")
                answer = self.generate_answer(user_input, marks, chapter)
